...
public class ThreadDiffCalculator {
  ...
  public void calculateDiff(List<ThreadDetail> oldSnapShot, List<ThreadDetail> newSnapShot, File diffReportFile) {
    ...
    List<String> deletedThreads = printDeletedThreads(oldSnapShot, newSnapShot, oldNidToThreadMapping, newNidToThreadMapping);
    List<String> addedThreads = printAddedThreads(oldSnapShot, newSnapShot, oldNidToThreadMapping, newNidToThreadMapping);
    List<String> cpuNotIncreased = printIfCpuNotIncreased(oldSnapShot, newSnapShot, oldNidToThreadMapping, newNidToThreadMapping);
    List<String> elapsedNotIncreased = printIfElapsedNotIncreased(oldSnapShot, newSnapShot, oldNidToThreadMapping, newNidToThreadMapping);
    printChangeInPoolSize(oldSnapShot, newSnapShot);

    List<String> buffer = new ArrayList<>();

    if (!deletedThreads.isEmpty()) {
      buffer.add("Deleted threads: " + StringUtils.join(deletedThreads, ","));
    }

    if (!addedThreads.isEmpty()) {
      buffer.add("Added threads: " + StringUtils.join(addedThreads, ","));
    }
